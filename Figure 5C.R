...
counts_gns = read.csv(paste(raw_count_file_dir,'Control_Telescope_output_HML2_GENES.csv',sep='/'))

###Get the HERV sheet
raw_count_file_dir_HERV  = file.path(results_dir,'telescope', 'counts_matrices', 'raw',Genome,'All_Proviruses')

counts_HERV = read.csv(paste(raw_count_file_dir_HERV,'Control_Telescope_output_hg38_p14_RepeatMasker_HS_LS_Reports_LS.csv',sep='/'))

counts_gns <- counts_gns[,intersect(names(counts_gns), names(counts_HERV))]
#rm(counts_HERV)

coldata = read.csv(paste(results_dir,'runTable_Control.csv',sep='/'))
...
countmat_gns = round(data.matrix(counts_gns[,-1]))
rownames(countmat_gns) = counts_gns$X
rm(counts_gns)

gns_Norm_counts_mat_unfiltered <- GetNormalizedMat(countmat_gns, MedianNorm(countmat_gns))

##### Normalize GBM_counts_HERV

countmat_HERVs = round(data.matrix(counts_HERV[,-1]))
rownames(countmat_HERVs) = counts_HERV$X
rm(counts_HERV)

HERVs_Norm_counts_mat_unfiltered <- GetNormalizedMat(countmat_HERVs, MedianNorm(countmat_HERVs))
...
Norm_counts_mat_unfiltered <- rbind(HERVs_Norm_counts_mat_unfiltered,gns_Norm_counts_mat_unfiltered)
...
# identifying HERVs based on underscores anywhere in the name
