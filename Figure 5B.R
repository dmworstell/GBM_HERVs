...
###FILTER TO ONLY TAKE TCGA DATA ###
coldata<- subset(coldata,study_name=='TCGA')

just_using_runs <- coldata$run[(coldata$study_name == 'TCGA')]

library(EBSeq)
#####Normalize gbm_counts

#Convert counts to matrix
gbm_countmat = round(data.matrix(gbm_counts[,-1]))
rownames(gbm_countmat) = gbm_counts$X
rm(gbm_counts)

gbm_countmat <- gbm_countmat[, just_using_runs]

GBM_Norm_counts_mat_unfiltered <- GetNormalizedMat(gbm_countmat, MedianNorm(gbm_countmat))

##### Normalize GBM_counts_HERV

#Convert counts to matrix
gbm_HERV_countmat = round(data.matrix(GBM_counts_HERV[,-1]))
rownames(gbm_HERV_countmat) = GBM_counts_HERV$X
rm(GBM_counts_HERV)

###FILTER TO ONLY TAKE TCGA DATA ###

gbm_HERV_countmat <- gbm_HERV_countmat[, just_using_runs]


GBM_HERV_Norm_counts_mat_unfiltered <- GetNormalizedMat(gbm_HERV_countmat, MedianNorm(gbm_HERV_countmat))

Norm_counts_mat_unfiltered <- rbind(GBM_HERV_Norm_counts_mat_unfiltered,GBM_Norm_counts_mat_unfiltered)
...
## FILTERED
keep = which(rowMeans(Norm_counts_mat_unfiltered) > 30)
...
##Assess normality##
# Extract expression data for "HML-2_1q22"
prv_expression <- Norm_counts_mat["HML-2_1q22", ]
...
HML2_1q22_expression <- Norm_counts_mat["HML-2_1q22", ]

lower_quantile_value <- quantile(HML2_1q22_expression, probs = 0.25) # Approximation for theoretical quantile -1
upper_quantile_value <- quantile(HML2_1q22_expression, probs = 0.75) # Approximation for theoretical quantile 1

# Identify samples within these quantile values
samples_within_range <- which(HML2_1q22_expression >= lower_quantile_value & HML2_1q22_expression <= upper_quantile_value)

# Subset Norm_counts_mat to retain only those samples
subset_Norm_counts_mat <- Norm_counts_mat[, samples_within_range]

#Assess normality
prv_expression <- subset_Norm_counts_mat["HML-2_1q22", ]
# Generate a Q-Q plot
qqnorm(prv_expression)
qqline(prv_expression, col = "red")  # Adds a reference line
...
hervs_of_interest <- rownames(countmat_filtered_cd)[is_herv & grepl("HML-2_1q22", rownames(countmat_filtered_cd))]

# Keep only HERVs of interest and all genes
filtered_countmat <- countmat_filtered_cd[rownames(countmat_filtered_cd) %in% hervs_of_interest | !is_herv, ]

## Generate correlation matrices ##

# Extract only gene rows from the count matrix
genes_only <- filtered_countmat[!(rownames(filtered_countmat) %in% hervs_of_interest),]
...
# Loop through each HERV of interest
for (herv in hervs_of_interest) {
...
perform_go_analysis <- function(gene_list, orgDb, showCategory = 5, output_path) {
  ...
  if (length(ego) > 0) {  # Check if there are any enriched GO terms
    ...
    plot_enrichment <- custom_dotplot_enrichResult(ego, showCategory = showCategory, label_format = 20) + 
      xlab("Gene Ratio") +
      custom_theme(font.size = 12,label.font.size = 12)  # Use the custom theme
