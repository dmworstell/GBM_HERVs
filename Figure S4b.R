...
library(tidyverse)
library(ggplot2)
#source("/Users/Daniel/Documents/Med_School/G1-G4/Coffin_Lab/Thesis/Thesis_Project/RNA_Seq_Processing/TCGA_Analysis/scripts/SummarySE.R")
#source("//Users/Daniel/Documents/Med_School/G1-G4/Coffin_Lab/Thesis/Thesis_Project/RNA_Seq_Processing/TCGA_Analysis/scripts/SummarySEwithin.R")
#source("/Users/Daniel/Documents/Med_School/G1-G4/Coffin_Lab/Thesis/Thesis_Project/RNA_Seq_Processing/TCGA_Analysis/scripts/NormDataWithin.R")
...
#Ref_trans_path <- file.path(Base_dir,"Annotations","gencode.v41.annotation.gtf")
...
all_exonic_pvs_in_lncRNAs <- subset(Exonized_prvs_base, Exonized_prvs_base$transcript_type == "lncRNA")[,"Provirus"]
...
### Now obtain reference lncRNAs and PCGs from the GENCODE annotation
...
Ref_transcripts <- Ref_transcripts %>% 
  mutate(exon_number = str_extract(info_7, " exon_number \\d+") %>% str_extract("\\d+")) 

Ref_transcripts$exon_number <- as.numeric(Ref_transcripts$exon_number)
...
library(dplyr)
Ref_transcripts <- Ref_transcripts %>%
  dplyr::group_by(info_2) %>%
  dplyr::mutate(max_exons = max(exon_number, na.rm = TRUE)) %>%
  ungroup()
...
ref_lncRNAs <- subset(Ref_transcripts,info_5==" transcript_type lncRNA")

##Now, subset these so that only lncRNAs that had proviruses in them are considered
ref_lncRNAs <- ref_lncRNAs %>%
  mutate(gene_name = sub(" gene_name ", "", ref_lncRNAs$info_4))


ref_lncRNAs_chk <- subset(ref_lncRNAs, ref_lncRNAs$gene_name %in% unique(Exonized_prvs_base$gene_name))
ref_lncRNA_exons <- subset(ref_lncRNAs,V3=="exon")
...
lncRNA_exon_nums <- ref_lncRNA_exons[c("exon_number","max_exons")]
#lncRNA_exon_nums_formatted <- cbind(sub("exon_number ","",lncRNA_exon_nums$exon_number) , sub("terminal_exon ","", lncRNA_exon_nums$max_exons))
#lncRNA_exon_nums_formatted <- as.data.frame(cbind(lncRNA_exon_nums_formatted[,1], sub(";","", lncRNA_exon_nums_formatted[,2])))

names(lncRNA_exon_nums) <- c("exon_number","terminal_exon")
lncRNA_exon_nums <- as.data.frame(lapply(lncRNA_exon_nums, function(x) as.numeric(gsub('exon_number ','', x)) ) )

### Read in actually observed distribution info ###

all_pvs_in_lncRNAs <- rownames(filtered_tpm_countmat)[rownames(filtered_tpm_countmat) %in% all_exonic_pvs_in_lncRNAs]
Exonized_prvs_subset <- subset(Exonized_prvs_base, Provirus %in% all_pvs_in_lncRNAs)

all_pvs_in_lncRNAs_df <- as.data.frame(cbind(Exonized_prvs_subset$Provirus,Exonized_prvs_subset$exon_pos))

unique_pvs_in_lncRNAs <- unique(all_pvs_in_lncRNAs_df)

num_proviruses_in_lncRNA <-length(unique_pvs_in_lncRNAs$V1)

n_dups <- nrow(subset(as.data.frame(table( all_pvs_in_lncRNAs_df$V2 )),Freq>1))

observed_PRV_dist <- as.data.frame(table(all_pvs_in_lncRNAs_df$V2,useNA='no'))
...
for(bs in 1:num_iterations){
  ...
  for (i in 1:num_proviruses_in_lncRNA) {
    random_index=round(runif(1,min=1,max=num_proviruses_in_lncRNA))
    selected_exon <- lncRNA_exon_nums[random_index,]
...
#   xlab("Exon Location") +
#   ylab("Count") +
#   ggtitle("Observed HERV distribution in lncRNAs") +
#   theme_bw()
...
ggplot(as.data.frame(full_df), aes(x=Exon_Location, y=count,fill=ObservedOrSimulated)) + 
  ...
  xlab("Exon Location") +
  ylab("Count") +
  ggtitle("Simulated HERV distribution in lncRNAs") +
  theme_bw() +
...
ggplot(filtered_data_long, aes(x = Exon_Location, y = count)) +
  ...
  theme(
    ...
    axis.line.x.top = element_line(size = 1.5, color = "black"),  # Thick x and y axis lines
    axis.line.y.right = element_line(size = 1.5, color = "black"),  # Thick x and y axis lines
